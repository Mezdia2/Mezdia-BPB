...
function setActiveConnections(value) {
...
function normalizeConfig(rawConfig = {}) {
    ...
    const account = {
        ...
        ...(migratedUser ? {
            ...
            disabledReason: migratedUser.disabledReason || null,
            disabledAt: migratedUser.disabledAt || null,
...
async function handleAuth(request, hostName, ctx, env) {
    ...
    const usageData = {};
    for (const [k, v] of uuidUsage.entries()) usageData[k] = v;
    return jsonResponse({
        success: true,
        config: publicConfig(),
...
<head>
  ...
  <style>
    :root{color-scheme:dark;--bg:#0c1116;--panel:#141b22;--panel2:#10161d;--line:#2a3540;--text:#e8edf2;--muted:#92a0ad;--accent:#33d17a;--warn:#ffb454;--bad:#ff6b6b}
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
    header{height:58px;display:flex;align-items:center;gap:16px;padding:0 22px;border-bottom:1px solid var(--line);background:#0f151b;position:sticky;top:0;z-index:2}
    h1{font-size:17px;margin:0;font-weight:750}main{display:grid;grid-template-columns:260px 1fr;min-height:calc(100vh - 58px)}
    nav{border-right:1px solid var(--line);background:#0f151b;padding:18px}nav button{width:100%;text-align:left;border:0;background:transparent;color:var(--muted);padding:10px 12px;border-radius:6px;margin-bottom:4px;cursor:pointer;font-weight:650}
    nav button.active,nav button:hover{background:#17212a;color:var(--text)}section{display:none;padding:24px;max-width:1180px}section.active{display:block}
    .grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.card{background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:16px}
    .full{grid-column:1/-1}h2{font-size:20px;margin:0 0 14px}h3{font-size:14px;margin:0 0 12px;color:#cfd7df;text-transform:uppercase;letter-spacing:.04em}
    label{display:block;color:var(--muted);font-size:12px;font-weight:700;margin:12px 0 6px}input,select,textarea{width:100%;background:#0d1319;color:var(--text);border:1px solid var(--line);border-radius:6px;padding:10px;font:inherit}
    textarea{min-height:96px;resize:vertical}.row{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.actions{display:flex;gap:10px;flex-wrap:wrap;margin-top:16px}
    button.primary,button.secondary,button.danger{border:0;border-radius:6px;padding:10px 13px;font-weight:750;cursor:pointer}
    button.primary{background:var(--accent);color:#04120a}button.secondary{background:#24313d;color:var(--text)}button.danger{background:var(--bad);color:#250606}
    .pill{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--line);background:#0f151b;border-radius:999px;padding:5px 9px;color:var(--muted);font-size:12px}.ok{color:var(--accent)}.warn{color:var(--warn)}.bad{color:var(--bad)}
    code,pre{background:#0b1015;border:1px solid var(--line);border-radius:6px}code{padding:2px 5px}pre{padding:12px;overflow:auto;white-space:pre-wrap}.muted{color:var(--muted)}.hidden{display:none!important}
    #login{position:fixed;inset:0;display:flex;align-items:center;justify-content:center;background:var(--bg);z-index:5}.login-card{width:min(420px,calc(100vw - 32px));background:var(--panel);border:1px solid var(--line);border-radius:8px;padding:22px}
    table{width:100%;border-collapse:collapse}td,th{border-bottom:1px solid var(--line);padding:9px;text-align:left}th{color:var(--muted);font-size:12px}
    @media(max-width:800px){main{grid-template-columns:1fr}nav{display:flex;overflow:auto;border-right:0;border-bottom:1px solid var(--line)}nav button{white-space:nowrap;width:auto}.grid,.row{grid-template-columns:1fr}section{padding:16px}}
  </style>
</head>
<body>
  <div id="login">
    <div class="login-card">
      <h2>${BRAND_NAME}</h2>
      <p class="muted">Sign in with the panel password. Programmatic access uses <code>MEZDIA_API_KEY</code>.</p>
      ${hasDB ? "" : `<p class="bad">IOT_DB is not bound. Settings cannot persist.</p>`}
      ...
      <p id="login-error" class="bad hidden">Authentication failed.</p>
    </div>
  </div>
  <header>
    <h1>${BRAND_NAME}</h1>
  ...
  <main>
    ...
    <section id="overview" class="active">
      <h2>Overview</h2>
      <div class="grid">
        <div class="card"><h3>Subscription</h3><p><code id="sub-url"></code></p><div class="actions"><button class="secondary" onclick="copyText('sub-url')">Copy URL</button></div></div>
        <div class="card"><h3>Traffic</h3><p id="traffic-summary" class="muted">Loading</p></div>
        <div class="card"><h3>Runtime</h3><p id="runtime-summary" class="muted">Loading</p></div>
        <div class="card"><h3>Account Status</h3><p id="account-summary" class="muted">Loading</p></div>
      </div>
    </section>
    <section id="account">
      ...
      <div class="card">
        <div class="row">
          <div><label>Name</label><input id="account-name"></div>
          <div><label>Status</label><select id="account-status"><option value="active">Active</option><option value="paused">Paused</option></select></div>
        </div>
        <label>Notes</label><textarea id="account-notes"></textarea>
        <div class="row">
          <div><label>Traffic limit in GB (0 means unlimited)</label><input id="account-limit-gb" type="number" min="0" step="0.01"></div>
          <div><label>Expiry timestamp in milliseconds (0 means never)</label><input id="account-expiry" type="number" min="0" step="1"></div>
        </div>
        <div class="row">
          <div><label>Account clean IP override</label><textarea id="account-clean-ip"></textarea></div>
          <div><label>Account proxy IP override</label><textarea id="account-proxy-ip"></textarea></div>
        </div>
        <div class="row">
          <div><label>Account mode override</label><select id="account-mode"><option value="">Use global mode</option><option value="alpha">Alpha</option><option value="beta">Beta</option><option value="both">Both</option></select></div>
          <div><label>Account ports override</label><input id="account-ports" placeholder="443,8443"></div>
        </div>
        <label>Maximum generated configs for the account</label><input id="account-max-configs" type="number" min="0" step="1">
        <div class="actions"><button class="primary" onclick="saveAccount()">Save account</button><button class="danger" onclick="resetTraffic()">Reset traffic</button></div>
      </div>
    </section>
    <section id="settings">
      <h2>Settings</h2>
      <div class="grid">
        <div class="card">
          <h3>Core</h3>
          <label>Panel name</label><input id="cfg-name">
          <label>API route</label><input id="cfg-apiRoute">
          <label>Panel password</label><input id="cfg-masterKey" type="password">
          <label>Device UUID</label><input id="cfg-deviceId">
          <label>Mode</label><select id="cfg-mode"><option value="alpha">Alpha</option><option value="beta">Beta</option><option value="both">Both</option></select>
          <label>Socket ports</label><input id="cfg-socketPorts">
          <label>Client fingerprint</label><input id="cfg-agent">
        </div>
        <div class="card">
          <h3>Network</h3>
          <label>Maintenance hosts</label><textarea id="cfg-maintenanceHost"></textarea>
          <label>Clean IPs</label><textarea id="cfg-cleanIps"></textarea>
          <label>Backup relay / proxy IPs</label><textarea id="cfg-backupRelay"></textarea>
          <label>Custom relay override</label><input id="cfg-customRelay">
          <label>Slave nodes</label><textarea id="cfg-slaveNodes"></textarea>
          <label>DNS over HTTPS endpoint</label><input id="cfg-customDns">
          <label>Resolve IP</label><input id="cfg-resolveIp">
          <label>Metric node</label><input id="cfg-metricNode">
        </div>
        <div class="card">
          <h3>Generation</h3>
          <label>Name prefix</label><input id="cfg-namePrefix">
          <label>Name strategy</label><input id="cfg-nameStrategy">
          <label>Subscription User-Agent allowlist</label><input id="cfg-subUserAgent">
          <label>Custom panel URL</label><input id="cfg-customPanelUrl">
          <label><input id="cfg-enableOpt1" type="checkbox"> Enable TCP fast open in generated configs</label>
          <label><input id="cfg-enableOpt2" type="checkbox"> Enable ECH option in generated configs</label>
          <label><input id="cfg-isPaused" type="checkbox"> Pause all stream traffic</label>
        </div>
        <div class="card">
          <h3>Backup</h3>
          <div class="actions"><button class="secondary" onclick="exportConfig()">Export JSON</button><button class="secondary" onclick="importConfig()">Import JSON</button></div>
          <label>Import / raw config JSON</label><textarea id="raw-config" style="min-height:210px"></textarea>
        </div>
      </div>
    ...
    <section id="api">
      <h2>API Usage</h2>
      <div class="card">
        <p>All API requests use the Worker URL plus the Mezdia API key stored in <code>MEZDIA_API_KEY</code>.</p>
        <pre id="api-examples"></pre>
      </div>
    </section>
    <section id="logs">
      <h2>Logs</h2>
      <div class="card"><table><thead><tr><th>Time</th><th>Type</th><th>Detail</th></tr></thead><tbody id="log-body"></tbody></table></div>
    </section>
  </main>
  <script>
    ...
    function render(data){
      ...
      document.getElementById('status-pill').textContent = data.account.status;
      document.getElementById('sub-url').textContent = data.account.subscriptionUrl;
      document.getElementById('traffic-summary').textContent = stats.totalGB + ' GB used, ' + (stats.totalLimitGB || 'unlimited') + ' GB limit. Daily: ' + stats.dailyGB + ' GB.';
      document.getElementById('runtime-summary').textContent = 'Device ' + data.deviceId + '. Version ' + data.version + '. Active stream cache: ' + Object.keys(data.usage || {}).length + '.';
      document.getElementById('account-summary').textContent = data.account.name + ' is ' + data.account.status + '.';
      fillSettings();
      fillAccount();
    ...
    function fillSettings(){
      ['name','apiRoute','masterKey','deviceId','mode','socketPorts','agent','maintenanceHost','cleanIps','backupRelay','customRelay','slaveNodes','customDns','resolveIp','metricNode','namePrefix','nameStrategy','subUserAgent','customPanelUrl','enableOpt1','enableOpt2','isPaused'].forEach(k=>setValue('cfg-'+k,config[k]));
    ...
    function fillAccount(){
      ...
      setValue('account-limit-gb',account.usage.totalLimitGB || 0);setValue('account-expiry',account.usage.expiryMs || 0);
    ...
    async function saveSettings(){
      const patch={};
      ['name','apiRoute','masterKey','deviceId','mode','socketPorts','agent','maintenanceHost','cleanIps','backupRelay','customRelay','slaveNodes','customDns','resolveIp','metricNode','namePrefix','nameStrategy','subUserAgent','customPanelUrl','enableOpt1','enableOpt2','isPaused'].forEach(k=>patch[k]=getValue('cfg-'+k));
      const res = await fetch(baseRoute + '/api/sync',{method:'POST',headers:headers(),body:JSON.stringify({key:sessionKey,config:patch})});
      if(res.ok){alert('Settings saved. If the API route changed, open /' + patch.apiRoute + '/dash.');location.reload();}else alert('Save failed');
    }
    async function saveAccount(){
      const patch={account:{name:getValue('account-name'),status:getValue('account-status'),notes:getValue('account-notes'),cleanIp:getValue('account-clean-ip')||null,proxyIp:getValue('account-proxy-ip')||null,userMode:getValue('account-mode')||null,userPorts:getValue('account-ports')||null,maxConfigs:getValue('account-max-configs')||null},trafficLimitGB:Number(getValue('account-limit-gb')||0),expiryMs:Number(getValue('account-expiry')||0)};
      const res = await fetch(baseRoute + '/api/sync',{method:'POST',headers:headers(),body:JSON.stringify({key:sessionKey,config:patch})});
      if(res.ok){alert('Account saved.');location.reload();}else alert('Save failed');
    }
    async function resetTraffic(){if(!confirm('Reset account traffic counters?'))return;const res=await fetch(baseRoute + '/api/sync',{method:'POST',headers:headers(),body:JSON.stringify({key:sessionKey,resetTraffic:true})});if(res.ok)location.reload();}
    function exportConfig(){document.getElementById('raw-config').value=JSON.stringify(config,null,2);}
    async function importConfig(){try{const next=JSON.parse(document.getElementById('raw-config').value);const res=await fetch(baseRoute + '/api/sync',{method:'POST',headers:headers(),body:JSON.stringify({key:sessionKey,config:next})});if(res.ok)location.reload();else alert('Import failed');}catch(e){alert('Invalid JSON');}}
